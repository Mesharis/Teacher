...
import '../../../models/repeat_duration_model.dart';
import '../../../models/repeat_model.dart';
...
class AddTimeslotController extends GetxController {
  ...
  void onInit() {
    ...
    if (editedTimeSlot != null) {
      ...
      newDateTime = date;
      print('current date time : ${newDateTime.isUtc}');
  ...
  void addTimeslot() async {
    try {
      DateTime formattedDateTime = DateTime(newDateTime.year, newDateTime.month, newDateTime.day, 24);
      if (formattedDateTime.compareTo(DateTime.now()) < 0) {
        Fluttertoast.showToast(
            msg: 'Date Time is in the past'.tr,
      ...
      if (validationSuccess) {
        ...
        Fluttertoast.showToast(
            msg: 'Success adding Timeslot'.tr,
            backgroundColor: Colors.white,
            textColor: Colors.black);
  ...
  List<DateTime> _generateRepeatTimeslot(
    ...
    switch (repeatTimeslot.repeat) {
      case Repeat.EVERY_DAY:
        int daysInMonth = Jiffy.parseFromDateTime(newDateTime).daysInMonth;
        for (int i = 1; i < daysInMonth; i++) {
          DateTime dateRepeat = Jiffy.parseFromDateTime(newDateTime).add(days: i).dateTime;
      ...
      case Repeat.SAME_DAY_EVERY_WEEK:
        ...
        for (int i = 1; i <= week; i++) {
          DateTime dateRepeat = Jiffy.parseFromDateTime(newDateTime).add(weeks: i).dateTime;
          listDateTimeRepeat.add(dateRepeat);
        }
        break;
      case Repeat.SAME_DAY_EVERY_MONTH:
        for (int i = 1; i <= repeatDuration.month; i++) {
          DateTime dateRepeat = Jiffy.parseFromDateTime(newDateTime).add(months: i).dateTime;
          listDateTimeRepeat.add(dateRepeat);
        }
        break;
      default:
  ...
  Future deleteRepeatedTimeslot() async {
    TimeSlotService().deleteRepeatedTimeSlot(editedTimeSlot!).then((value) {
      Fluttertoast.showToast(
          msg: 'Success delete timeslot'.tr,
          backgroundColor: Colors.white,
          textColor: Colors.black);
      appointController.updateEventsCalendar();
      Get.back(closeOverlays: true);
