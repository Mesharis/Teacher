...
class TimeSlotService {
  ...
  Future<String> saveDoctorTimeslot(
    ...
    try {
      if (isParentTimeslot) {
        var timeSlotSaved = await FirebaseFirestore.instance.collection('DoctorTimeslot').add(TimeSlot().toMap(timeSlot));
  ...
  Future updateRepeatedTimeSlot(TimeSlot timeSlot) async {
    try {
      ...
      for (var item in docRef.docs) {
        ...
        print('timeslot hour : ${timeSlot.timeSlot}');
  ...
  Future<List<OrderModel>> getOrderedTimeSlot(limit) async {
    try {
      var doctor = await DoctorService().getDoctor();
      var documentRef = FirebaseFirestore.instance.collection('Order').where('doctorId', isEqualTo: doctor!.doctorId);
      var documentSnapshot = limit == null ? await documentRef.get() : await documentRef.limit(limit).get();

      if (documentSnapshot.docs.isEmpty) {
        return [];
      }

      List<OrderModel> listTimeslot = documentSnapshot.docs.map((doc) {
        var data = doc.data();
        data['orderId'] = doc.reference.id;
