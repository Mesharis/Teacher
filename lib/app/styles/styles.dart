import 'package:flutter/material.dart';

class Styles {
  static const greyTextColor = Color(0xFF8A8A8E);
  static const cardLighBluecolor = Color(0xFFECF9FC);
  static const secondaryColor = Color(0xFFFFB703);
  static const primaryColor = Color(0xFF03045E);
  static const whiteGreyColor = Color(0xFFF9F9F9);

  static final homeTitleStyle = TextStyle(
      fontWeight: FontWeight.w500, fontSize: 15);

  static final homeSubTitleStyle = TextStyle(
      fontWeight: FontWeight.w500, fontSize: 15, color: greyTextColor);

  static final appointmentDetailTextStyle =
      TextStyle(fontWeight: FontWeight.w500, fontSize: 18);

  static final appBarTextStyle = TextStyle(
      fontWeight: FontWeight.w600,
      fontSize: 20,
      color: primaryColor,
      fontFamily: 'Almarai');
  static final greyTextInfoStyle = TextStyle(
      fontWeight: FontWeight.w500, fontSize: 14, color: greyTextColor);
}
